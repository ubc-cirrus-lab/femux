...
def pipeline(n_input_steps: int, n_pred_steps: int, invocations: list[int],
             num_days: int) -> Tuple[pd.DataFrame, dict, dict]:
    df = load_dataset(invocations, num_days=num_days)
...
def load_dataset(invocations: list[int],
                 num_days: int) -> pd.DataFrame:
    df = pd.DataFrame()
    df["invocation_rate"] = np.array(invocations).astype(np.float64)
...
if __name__ == "__main__":
    #pipeline()
    load_dataset(hash_function="d9272e4b43e348badb6af384efcc10835704037e1cf1d215241a9a7a949eaa17", 
                 big_df=pd.read_pickle("../../data/azure/preproc_data/invocation_data/preprocessed_data_00.pickle"), num_days=7)
