"""
taken from: https://github.com/ChadFulton/statsmodels/blob/setar/statsmodels/tsa/setar_model.py
...
from forecasters.linear_model import OLS, OLSResults
from forecasters.decorators import (resettable_cache, cache_readonly, cache_writable)
import statsmodels.base.wrapper as wrap
import forecasters.setar_utils
...
class SETAR(OLS, tsbase.TimeSeriesModel):
    ...
    def __init__(self, endog, order, ar_order,
        ...
        if thresholds != None and not (len(thresholds) + 1 == order):
        ...
        self.max_delay = max_delay if max_delay != None else ar_order
    ...
    def _get_predict_end(self, end):
        ...
        """
        num_observations = len(self.data.orig_endog)
        if end is None:
            end = num_observations
            out_of_sample = 0
        elif not isinstance(end, int):
            raise ValueError("Invalid start value")
        else:
            out_of_sample = max(0, end - num_observations)
            end = num_observations
    ...
    def _select_hyperparameters_grid(self, thresholds, threshold_grid_size,
        ...
        if delay_grid is None or delay_grid[0] is None:
    ...
    def select_hyperparameters(self, threshold_grid_size=None, maxiter=100):
        ...
        delay_grid = [self.delay] if self.delay != None else None
...
class SETARResults(OLSResults, tsbase.TimeSeriesModelResults):
    ...
    def forecast(self, steps=1, method='mc', reps=100, initial=None):
        """
        Out-of-sample forecasts
        Parameters
        ----------
        ...
        return self.model.forecast(self.model.delay, self.model.thresholds,
                                   self.params, steps, initial=initial, method=method,
    ...
    def summary(self, yname=None, title=None, alpha=.05):
        ...
        title : string, optional
            Title for the top table. If != None, then this replaces the
            default title
        alpha : float
            significance level for the confidence intervals
        Returns
